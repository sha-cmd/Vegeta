...
\begin{table}[H]
  \centering
    ...
    \hline
    arbres & espece & variété  & haut/moy & remarquable & lieu & pourcent \\
    \hline
...
class Report:
    ...
    def data_builder(self):
        ...
        self.quant_gen_filtered = pd.DataFrame()
        self.genres_list = list()
        self.especes_list = list()
        self.varietes_list = list()
        #elf.quant_gen = pd.read_excel('quant_gen.xlsx')
        ...
        data['sta_dev_num'] = data['stade_developpement'].map({'P': 2, 'M': 5, 'JA': 3, 'A': 4, 'J': 1})
        self.d_filtered = data.loc[((data["hauteur_m"] < 30) & (data["hauteur_m"] > 0)) \
                     & ((data["circonference_cm"] < 400) & (data["circonference_cm"] > 0))].copy()
        self.d = data.copy()
        self.genres_list = list(self.d['genre'].unique())
        self.especes_list = list(self.d['espece'].unique())
        self.varietes_list = list(self.d['espece'].unique())
        ...
        q5 = """SELECT  genre,
                        ...
                        FROM d GROUP BY d.genre"""
        self.quant_gen = ps.sqldf(q5, locals())
        self.quant_gen_filtered = pd.read_excel('quant_gen.xlsx')
    def plot(self):
    ...
    def graph_sta_dev_num(self):
        ...
        for genre in self.genres_list:
            ...
            ax = sns.violinplot(order=dom_order_list, x="domanialite", y="sta_dev_num", data=self.d.loc[self.d['genre'] == genre], inner=None, scale='width')
            ax = sns.stripplot(order=dom_order_list, x="domanialite", y="sta_dev_num", data=self.d.loc[self.d['genre'] == genre]).set(title=genre.title())
    ...
    def graph_hauteur_m(self):
        ...
        for genre in self.genres_list:
            f = plt.figure(figsize=(20, 10))
            try:
                ax = sns.violinplot(order=[1,2,3,4,5], x="sta_dev_num", y="hauteur_m", data=self.d_filtered.loc[self.d_filtered['genre'] == genre], inner=None, scale='width')
                ax = sns.stripplot(order=[1,2,3,4,5], x="sta_dev_num", y="hauteur_m", data=self.d_filtered.loc[self.d_filtered['genre'] == genre]).set(title=genre.title())
            except:
                pass
    ...
    #def grah_circonference_cm(self)
